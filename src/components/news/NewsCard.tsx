...
export default function NewsCard({ article, variant = 'default' }: NewsCardProps) {
  const formattedDate = format(new Date(article.created_at), "dd 'de' MMMM, yyyy", {
    locale: ptBR,
  });
  ...
  if (variant === 'featured') {
    return (
      <article className="group relative overflow-hidden rounded-lg bg-card">
        <Link to={`/noticia/${article.id}`} className="block">
          {/* Image */}
  ...
  if (variant === 'horizontal') {
    return (
      <article className="group news-card rounded-lg overflow-hidden">
        <Link to={`/noticia/${article.id}`} className="flex gap-4">
          {/* Image */}
